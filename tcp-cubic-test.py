...
class ConsoleApp(Frame):
    ...
    # TODO: For each client host use: consoles[i].sendCmd( '???? ' + ?? +  '> ' + file_path + str(i) + ' 2>&1')
        for i in range(4, 8):
            ...
            consoles[i].sendCmd('iperf3 -c ' + ip + ' --time 60 > ' + file_path + str(i) + ' 2>&1')
        # print(self.consoles.keys())
        switch_consoles = self.consoles['switches'].consoles
        time.sleep(20)
        switch_consoles[0].node.cmd('sudo ifconfig s1-eth1 down')
        time.sleep(2)
        switch_consoles[0].node.cmd('sudo ifconfig s1-eth1 up')
        time.sleep(18)
        switch_consoles[0].node.cmd('sudo ifconfig s1-eth1 down')
        time.sleep(2)
        switch_consoles[0].node.cmd('sudo ifconfig s1-eth1 up')
        #ip = consoles[4 - count / 2].node.IP()
        #consoles[4].sendCmd('iperf3 -c ' + ip + ' --udp --time 60 > ' + file_path + str(4) + ' 2>&1')
